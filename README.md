...
> what's still pending. Stage 4 (dashboard): demo mode built, tested, and
> screenshotted (see [Stage 4: dashboard](#stage-4-dashboard)); live mode
> not started. Real numbers below, not projections.
...
> **Status: demo mode built, tested, and screenshotted. Live mode
> (health-check-based switching, real repo/SHA input, backend glue) is a
> separate phase not yet started** — built and reviewed in this order
> deliberately, per instruction, so the honest prediction cards could be
> checked against real fixture data before any live path was wired in.

A React (Vite) dashboard at `web/`. Demo mode requires no backend at all —
4 real captured model outputs are bundled directly in the frontend.
...
**What's real:** every one of the 4 demo examples is a genuine inference
from the actual trained model — nothing in `web/src/data/demoFixtures.json`
is hand-typed, and it's regenerated by
[scripts/09_generate_demo_fixtures.py](scripts/09_generate_demo_fixtures.py),
never hand-edited. One example (the "healthy history" one) is backed by a
real live GitHub extraction end-to-end; the other three use hand-specified
input vectors — identical to existing test fixtures for two of them — run
through the real model. Full provenance for each, including why the 4th
example isn't backed by a second live pull (this session's unauthenticated
GitHub rate limit ran out), is in `web/README.md` and inside the fixture
JSON itself.
...
shown, lives inside a visually de-emphasized secondary line. The
`cold_start` card uses a completely different visual pattern (dashed
border, neutral blue, its own badge shape) from every risk-tier card, so
it can never be mistaken for one. The cross-CI-system caveat is a
persistent banner at the top of the page, not a tooltip.

**Deviation flagged:** the brief named a "frontend-design" skill to load
before styling — no such skill exists in this environment. Styled by hand
instead; full reasoning in `web/README.md`.
